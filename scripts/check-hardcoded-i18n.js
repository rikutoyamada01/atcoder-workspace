...
files.forEach((filePath) => {
  ...
  lines.forEach((line, index) => {
    ...
    if (filePath.endsWith('.html') && (trimmed.includes('data-i18n=') || trimmed.includes('data-i18n-placeholder='))) {
      return;
    }
...
console.log('--------------------------------------------------');
if (totalMatches > 0) {
  console.log(`⚠️ Found ${totalMatches} hardcoded Japanese string(s) across ${totalFilesWithMatches} file(s).`);
  console.log('   Please replace these with i18nProvider.t(...) or chrome.i18n.getMessage(...) for 100% i18n coverage.');
} else {
